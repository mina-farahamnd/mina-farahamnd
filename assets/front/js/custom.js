    ...
    window.clearInterval(timer);
      ...
      success: function (data) {
        if ((data.errors)) {
          ...
          $this.find('.alert-danger').show();
          $this.find('.alert-danger ul').html('');
            $this.find('.alert-danger p').html('');
            for (var error in data.errors) {
            $('.signin-form .alert-danger p').append(data.errors[error] + "<br>");
          }
            if(data.status == "-1"){
                var time = 120;
          ...
          $this.find('.alert-danger').hide();
          $this.find('.alert-success').show();
          $this.find('.alert-success p').html('در حال ورود به حساب کاربری !');
          if (data == 1) {
            location.reload();
          } else {
            window.location = data;
        ...
        if (data == 1) {
          ...
          if ((data.errors)) {
            ...
            $this.find('.alert-danger ul').html('');
              $this.find('.alert-danger p').html('');
            for (var error in data.errors) {
              $this.find('.alert-danger p').append(data.errors[error] + "<br>");
            }
            $this.find('button.submit-btn').prop('disabled', false);
          } else {
            $this.find('.alert-info').hide();
  ...
  $(document).on('click', '.add-to-wish', function(){
      if (isLogin){
          $.get( $(this).data('href') , function( data ) {
              ...
              if(data[0] == 1) {
                  toastr.success("محصول به لیست علاقه مندی اضافه شد.");
                  $('#wishlist-count').html(data[1]);
              }
              else {
                  toastr.error("محصول قبلا به لیست علاقه مندی اضافه شده است.");
              }

          });
      }else{
          toastr.error("برای اضافه کردن وارد حساب کاربری خود شوید.");
          return false;
      }
