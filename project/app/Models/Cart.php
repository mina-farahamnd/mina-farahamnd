...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key, $keys, $values,$method) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        if($item->type == 'Physical')
        {
        ...
        if($method){
            if ($color){
                $color_inventory = collect($item->color)->where("color_values" , "#" . $color)->first()->color_inventory;
                $total_qty = $this->totalColorQty($color);
                if(($total_qty - $storedItem['qty'] + $qty) > (int)$color_inventory){
                    return ['msg' => 'موجودی انبار کافی نیست.','error' => 'inventory'];
                }
            }elseif($item->stock){
                if(($qty) > $item->stock){
                    return ['msg' => 'موجودی انبار کافی نیست.','error' => 'inventory'];
            ...
            if ($color){
                $color_inventory = collect($item->color)->where("color_values" , "#" . $color)->first()->color_inventory;
                $total_qty = $this->totalColorQty($color);
                if(($qty + $total_qty) > $color_inventory){
                    return ['msg' => 'موجودی انبار کافی نیست.','error' => 'inventory'];
                }
            }elseif($item->stock){
                if(($storedItem['qty'] + $qty) > $item->stock){
                    return ['msg' => 'موجودی انبار کافی نیست.','error' => 'inventory'];
                }
            }
        ...
        if(!empty($item->color)){ 
        $storedItem['color'] = current($item->color);
        }  
        if(!empty($color)){
        $storedItem['color'] = $color;    
        }
...
// **************** REDUCING QUANTITY ENDS *******************
    ...
    public function totalColorQty($color)
    {
