...
class CheckoutController extends Controller
{
    ...
    public function checkout()
    {
        ...
        $vendor_packing_id = 0;
        Session::forget('price-with-discount');
            if (Session::has('currency')) 
            {
              $curr = Currency::find(Session::get('currency'));
            }
...
// If guest checkout is Deactivated then display pop up form with proper error message
    ...
    public function load_checkout_products()
    {
        $shipping_data  = DB::table('shippings')->where('user_id','=',0)->get();
        $addresses = Auth::user()->addresses;
        $oldCart = Session::get('cart');
        $cart = new Cart($oldCart);
        $products = $cart->items;
        return view('load.checkout',compact('products','addresses','shipping_data'));
    }
...
//            'address' => 'required|string',
        ...
        $address = Auth::user()->addresses()->where('id' , $request->selected_address)->first();
        if (!Session::has('cart') || !$address) {
        ...
        $totalPrice = Session::has('price-with-discount') ? Session::get('price-with-discount')[0] : $cart->totalPrice;
        $totalPrice = (int)$totalPrice;

        $shipping = Shipping::find($request->shipping_method);
        $price = $totalPrice + (int)$shipping->price;
        $order = new Order;
        $order['cart'] = utf8_encode(bzcompress(serialize($cart), 9));
        $order['totalQty'] = $cart->totalQty;
        $order['pay_amount'] = $price;
        $order['order_number'] = rand ( 1000 , 9999 ) . time();
        $order['payment_status'] = "Pending";
        $order['customer_name'] = $user->name . " " . $user->lastname;
        $order['customer_email'] = $user->email;
        $order['customer_country'] = 'ایران';
        ...
        $order['customer_zip'] = $address->post_code;
        $order['order_note'] = $request->order_notes;
        $order['coupon_code'] = Session::has('price-with-discount') ? Session::get('price-with-discount')[1] : '';
        $order['coupon_discount'] = Session::has('price-with-discount') ? Session::get('price-with-discount')[2] : '';
        $order['currency_sign'] = 'ریال';
        $order['currency_value'] = '1';
        $order['shipping_cost'] = $shipping->price;
        $order['shipping_title'] = $shipping->title;
        $order['tax'] = $gs->tax;

        if($request->payment_method == Order::ZARIN_PAL){
            $pay = new zarinpal;
            $order['txnid'] = $pay->pay((int)($price/10),$order['customer_email'] ,$request->phone);
            $order['method'] = 'درگاه بانکی زرین پال';

        }elseif($request->payment_method == Order::CASHONDELIVERY){
            $order['method'] = 'پرداخت درب منزل';
        }
        if($order = $user->orders()->save($order)){
            $order->tracks()->create([
                'title' => 'Pending',
            ...
            if($request->payment_method == Order::ZARIN_PAL){
                return redirect('https://www.zarinpal.com/pg/StartPay/' . $order['txnid']);
            }else{
                $cashoondelivery = true;
                return view('front.order-tracking',compact('cashoondelivery'));
            }
        }
    ...
    public function orderTracking(Request $request){
        ...
        $order = Order::where('txnid',$request->Authority)->first();

        if(isset($order->pay_amount)){
            $amount = (int)((Order::where('txnid',$request->Authority)->first()->pay_amount)/10);
            $gateway = new zarinpal();
            $result = $gateway->order($request,$amount);
        }else{
            return redirect()->route('front.index');
        }
        if($result){
            $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
            collect($cart->items)->map(function ($item, $key){
                $product = Product::findOrFail($item['item']['id']);
                if ($product->color){
                    $qty = $item['qty'];
                    $colors_collection = collect($product->color);
                    $color = $colors_collection->where("color_values" , "#" . $item['color'])->first();
                    $color->color_inventory -= $qty;
                    $color->color_inventory = max($color->color_inventory,0);
                    $product->color = json_encode($colors_collection);
                    $product->save();
            ...
            $tracks = $order->tracks()->update([
                'title' => 'Completed',
                'text' => 'Order Completed',
            ]);
            Session()->forget('cart');
            return view('front.order-tracking',compact('result','order'));
        }else{
            return view('front.order-tracking',compact('result'));
        }
