...
class CartController extends Controller
{
   ...
   public function addcart($id)
    ...
    public function addnumcart()
    {
        ...
        $size_price = (int)$_GET['size_price'];
        ...
        $color = str_replace('#','',$color);
        $cart = new Cart($oldCart);
        $msg = $cart->addnum($prod, $prod->id, $qty, $size,$color,$size_qty,$size_price,$size_key,$keys,$values,$method);
        if ($msg){
            return $msg;
        }
/*         if($cart->items[$id.$size.$color.str_replace(str_split(' ,'),'',$values)]['dp'] == 1)
    ...
    public function couponcheck()
    {
        ...
        $today = date('Y-m-d');
        $from = date('Y-m-d',strtotime($coupon->getOriginal('start_date')));
        $to = date('Y-m-d',strtotime($coupon->getOriginal('end_date')));
        if($from <= $today && $to >= $today)
        {
            if($coupon->status == 1)
            {
...
//                }
                ...
                if($coupon->type == 0)
                {
                    ...
                    $data[1] = $code;      
                    $data[2] = round($sub, 2);
                    Session::put('price-with-discount', $data);
                    $data[0] = (int)$total;
                    $data[1] = $code;
                    $data[2] = (int)$sub;
                    $data[3] = $coupon->id;
