...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      if (!empty($slug)) {
        $cat = Category::where('slug', $slug)->with(['attributes' => function ($query){
            $query->where('details_status',"1");
        }])->firstOrFail();
        $data['cat'] = $cat;
      }
      if (!empty($slug1)) {
        $subcat = Subcategory::where('slug', $slug1)->with(['attributes' => function ($query){
            $query->where('details_status',"1");
        }])->firstOrFail();
        $data['subcat'] = $subcat;
      }
      if (!empty($slug2)) {
        $childcat = Childcategory::where('slug', $slug2)->with(['attributes' => function ($query){
            $query->where('details_status',"1");
        }])->firstOrFail();
                                  ...
                                  ->when($stock, function($query, $stock) {
                                   ...
                                   ->when($sort, function ($query, $sort) {
                                      ...
                                      elseif($sort=='bestseller') {
                                        return $query->orderBy('total_sell', 'DESC');
                                      }
      ...
      $prods = $prods->where(function ($query) use ($cat, $subcat, $childcat, $request) {
                                              ...
                                              if (!empty($cat)) {
                                                foreach ($cat->attributes as $key => $attribute) {
                                                  ...
                                                  if (!empty($chFilters)) {
                                                    $flag = 1;
                                                      $query->where(function ($query) use ($cat,$chFilters){
    ...
    public function product($slug)
    {
        ...
        //return view('front.product2',compact('productt','curr','vendors'));
        return view('front.product',compact('productt'));
...
//        $product->update();
    ...
    // -------------------------------- PRODUCT COMMENT SECTION ----------------------------------------
        ...
        public function comment(Request $request)
        {
            ...
            $data[2] = $comment->created_at->formatDifference();
            $data[3] = $comment->text;
            $data[4] = $comments;
    ...
    // -------------------------------- PRODUCT REPLY SECTION ----------------------------------------
        ...
        public function reply(Request $request,$id)
        {
            ...
            $data[2] = $reply->created_at->formatDifference();
            $data[3] = $reply->text;
