...
public function index(Request $request)
{
    ...
    /*$last_products =  Product::where('status','=',1)->when($gs->affilate_product == 0,
                                                                                function($q){
                                                                                    return $q->where('product_type','=', 'normal');
    ...
    $feature_products =  Product::where('featured','=',1)->where('status','=',1)->when($gs->affilate_product == 0,
                                                                                function($q){
                                                                                    return $q->where('product_type','=', 'normal');
                                                                                })->orderBy('id','desc')->take($gs->max_recommend ? $gs->max_recommend : 9)->get();

    $all_products =  Product::where('status','=',1)->when($gs->affilate_product == 0,
        function($q){
            return $q->where('product_type','=', 'normal');
        })->get();
    ...
    $best_sellers_products =  Product::where('status','=',1)->when($gs->affilate_product == 0,
        function($q){
            return $q->where('product_type','=', 'normal');
        })->orderBy('sale','desc')->take($gs->max_recommend ? $gs->max_recommend : 8)->get();
    ...
    $discount_products =  Product::where('previous_price','!=',0)->WhereNotNull('previous_price')->when($gs->affilate_product == 0,
                                                                                function($q){
                                                                                    return $q->where('product_type','=', 'normal');
                                                                                })->orderBy('id','desc')->take($gs->max_discount ? $gs->max_discount : 10)->get();


    $slider_products =  Product::where('best','=',1)->where('status','=',1)->when($gs->affilate_product == 0,
                                                                                function($q){
                                                                                    return $q->where('product_type','=', 'normal');
                                                                                })->orderBy('id','desc')->take($gs->max_slider ? $gs->max_slider : 5)->get();


    $top_categories = Category::where('is_featured','=',1)->orderBy('id','desc')->take(7)->get();
    ...
    $large_banner = DB::table('banners')->where('type','=','Large')->orderBy('id','desc')->first();
    $ps = DB::table('pagesettings')->find(1);


    return view('front.index',compact('brands','best_sellers_products','all_products','feature_products','discount_products','last_products','hot_products','slider_products' ,'top_categories','top_small_banners','ps','bottom_small_banners','large_banner'));
        ...
        if(mb_strlen($slug,'utf-8') > 1){
            ...
            $prods = Product::where('name', 'like', '%' . $search . '%')
                            ...
                            /*->when($sort, function($query, $sort) {
                                ...
                                elseif($sort=='most_sell') {
                                    return $query->orderBy('total_sell', 'ASC');
                                }
                            })*/
                            ->take(10)
                            ->get();
