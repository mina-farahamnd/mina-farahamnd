...
//        return view('user.dashboard',compact('user'));
        ...
        $orders = Order::where('user_id','=', 22)->orderBy('id','desc')->get();
    ...
    public function profileupdate(Request $request)
    {
        ...
        $data->update($input);
        $msg = 'حساب کاربری با موفقیت به روزرسانی شد.';
        ...
        if ($request->cpass){
            if (Hash::check($request->cpass, $user->password)){
                if ($request->newpass == $request->renewpass){
                    ...
                    return response()->json(array('errors' => [ 0 => 'تکرار رمز عبور یکسان نیست.' ]));
                }
            }else{
    ...
    public function addressSubmit(Request $request)
    {
        ...
        $address = $request->only('address', 'city', 'province', 'post_code', 'phone', 'lat', 'lng' , 'title');
        if (Auth::user()->addresses()->create($address)){
            return response()->json('Address Create Successfully' , 201);
        }
