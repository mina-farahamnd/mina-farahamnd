...
class ProductController extends Controller
{
    ...
    public function uploadUpdate(Request $request,$id)
    {
        ...
        $image = $request->org_photo;
        list($type, $image) = explode(';', $image);
        list(, $image)      = explode(',', $image);
        $image = base64_decode($image);
        $org_photo = time().str_random(8).'.png';
        $path = 'assets/images/products/'.$org_photo;
        file_put_contents($path, $image);
        ...
        if($data->org_photo != null)
        {
            if (file_exists(public_path().'/assets/images/products/'.$data->org_photo)) {
                unlink(public_path().'/assets/images/products/'.$data->org_photo);
            }
        }


        $input['org_photo'] = $org_photo;
        $data->update($input);
        ...
        $data->update();
        return response()->json(['status'=>true,'photo' => $image_name , 'org_photo' => $org_photo]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'org_photo'      => 'required',
            'file'       => 'mimes:zip',
            "color_name.*" => 'required_with:color_inventory.*|nullable|string',
            "color_inventory.*" => 'required_with:color_name.*|nullable|integer',
            "color_values.*" => 'required_with:color_inventory.*|required_with:color_name.*|nullable',
            "attribute_value.*" => 'nullable|numeric',
            "price" => 'numeric',
            "previous_price" => 'nullable|numeric',
            "specification_keys.*" => 'nullable',
            "specification_values.*" => 'required_with:specification_keys.*|nullable',
        ];
        ...
        if($request->type == "Physical")
        {
            ...
            // Check Color
            /*if(!empty($request->color_name) && !empty($request->color_values)){
                if((array_filter($request->color_name)) && (array_filter($request->color_values))){
                    $input['color'] = json_encode(array_combine($request->color_name,$request->color_values));
                }
            }*/

            if(!empty($request->color_name) && !empty($request->color_values)){
                if((array_filter($request->color_name)) && $request->color_values && $request->color_inventory ){
                    $input_colors = collect($request->color_name)->map(function ($item, $key) use($request,$input){
                        return ["color_name" => $request->color_name[$key],"color_values" => $request->color_values[$key],"color_inventory" => $request->color_inventory[$key] ? $request->color_inventory[$key]: 0];
                    });
                    $input['color'] = json_encode($input_colors);
                }
            }else{
                $input['color'] = null;
            }
        ...
        if(!empty($request->specification_keys) && !empty($request->specification_values)){
            if((array_filter($request->specification_keys)) && (array_filter($request->specification_values))){
                $input['specifications'] = json_encode(array_combine($request->specification_keys,$request->specification_values));
            }
      ...
      // return $request;
        ...
        $rules = [
           'file'       => 'mimes:zip',
            ...
            "color_inventory.*" => 'required_with:color_name.*|nullable|integer',
            "color_values.*" => 'required_with:color_inventory.*|required_with:color_name.*|nullable',
            "attribute_value.*" => 'nullable|integer',
            "price" => 'integer',
        ...
        //Check Types
            ...
            unset($input['color_name']);
            unset($input['color_values']);
